...
var maketip = function (d) {
    var dt = new Date(d.Split);
    dt = dt.getMonth() + 1 + "/" + (dt.getFullYear());
    var tip = '<p class="tip3">' + d.name + '<p class="tip1">' + NumbType(d.value) + '</p> <p class="tip3">'+  dt+'</p>';
    return tip;}
...
function redraw() {
    ...
    var linedata = color.domain().map(function(name) {
        return {name: name,
            values: data.map(function(d) {
                return {name:name, Split: parseDate(d.Split), value: parseFloat(d[name],10)};
            })
        };
    });
    ...
    var x = d3.time.scale()
        .domain([
            d3.min(linedata, function(c) { return d3.min(c.values, function(v) { return v.Split; }); }),
            d3.max(linedata, function(c) { return d3.max(c.values, function(v) { return v.Split; }); })
        ])
        .range([0, width]);
    ...
    thegraph.selectAll("circle")
        ...
        .attr("cx", function(d,i){return x(d.Split)})
